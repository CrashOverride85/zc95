...
class PulseQueue:
    def __init__(self, start_time_us, bleak_client, pulse_char):
        ...
        self.pulse_queue = queue.Queue(18) # => (18 * 13 bytes) + 1 byte = up to 235 bytes per packet (using DLE)
        self.bleak_client = bleak_client
        self.pulse_char = pulse_char

    # If the first pulse in the queue is due in 22ms or less, send the queue now - don't wait for it to be full
    async def send_if_old(self, pattern_time_us):
        if self.pulse_count_in_queue > 0 :
            if self.first_pulse_in_queue_us - pattern_time_us < (22 * 1000):
                await self.send_pulses()

    # Send all queued pulses now
    async def send_pulses(self):
        # print("Send: " + str(self.pulse_count_in_queue))
        pulses = bytes([self.pulse_count_in_queue]) # first byte of packet is number of messages
        self.pulse_count_in_queue = 0
...
async def main(address):
    async with BleakClient(address) as client:
        ...
        async def timer_func():
            print("Send start")
            # Start message (more of a clock sync message). Tells the ZC95 that all future messages
            # will have a time_us relative to now.
            ...
            while True:
                pulse = BTMessagePulse(0x02, 130, 0, int(next_pulse_us)+(50 * 1000), 0xFF)  # pulse x ms from now
